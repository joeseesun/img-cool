...
const hasMoreThanOnePoint = (element) => {
    const points = element.points;
    return points && points.length > 1;
};
const hasResizeHandle = (board, element) => {
...
const getStrokeLineDash = (strokeStyle, strokeWidth) => {
    switch (strokeStyle) {
...
// 2. Add -> Subtract
const moveXOfPoint = (point, distance, direction = Direction.right) => {
...
const moveYOfPoint = (point, distance, direction = Direction.right) => {
    if (direction === Direction.bottom) {
        return [point[0] + distance, point[1]];
    }
    if (direction === Direction.top) {
...
const getXDistanceBetweenPoint = (point1, point2, isHorizontal) => {
    if (isHorizontal) {
...
const BOARD_TO_CANVAS_MAP = new WeakMap();
const ELEMENT_TO_SIZE_MAP = new WeakMap();
const getCanvasForBoard = (board) => {
    if (board) {
...
const getElementSize = (board, element, options, containerMaxWidth = 10000) => {
...
const updateElementSizeCache = (board, element, size) => {
    ELEMENT_TO_SIZE_MAP.set(element, size);
};
const clearElementSizeCache = (board, element) => {
    ELEMENT_TO_SIZE_MAP.delete(element);
};
function measureElement(board, element, options, containerMaxWidth = 10000) {
